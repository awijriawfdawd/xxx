...
__title__ = 'Devz UI Builder'
__avatar__ = 'https://cdn.discordapp.com/attachments/1155214110364008548/1155214244564971583/8eb9b9fc-283e-42f1-9b08-26c7fa00dc09.jpg'
__version__ = '1.9'
...
links = {
    "xpierroz_github": "discord.com/users/1112514275882307664",
...
def _github():
    with ui.card():
        with ui.row():
            ui.button("Open Devz Log", on_click=lambda: os.startfile(os.path.join(os.getcwd(), 'roselog.log')))
            ui.button("Open Devz Compile Log", on_click=lambda: os.startfile(os.path.join(os.getcwd(), 'rosecompile.log')))
        
        with ui.column():
            ui.markdown(f"<code>Message from {__devmsg__[0]}: {__devmsg__[1]}</code>")
            with ui.row():
                with ui.card_section():
                    ui.label("Devzhtz Contact").classes("text-h6")
